    ...
    return (
      <aside className="w-12 bg-white border-r border-slate-200 flex flex-col items-center py-3 space-y-4 text-slate-500 select-none shadow-xs">
        ...
        <button
          ...
          title={`${t('sidebar.allRecords')} (${categoryStats.total})`}
        >
        ...
        <button
          ...
          title={`${t('sidebar.starredFavorites')} (${categoryStats.favorites})`}
        >
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col h-full text-slate-700 text-xs select-none shadow-xs">
      ...
      <div className="p-3.5 border-b border-slate-100 space-y-2.5 bg-slate-50/50">
        <div className="flex items-center justify-between">
          <span className="font-semibold text-slate-900 text-xs flex items-center space-x-1.5">
            ...
            <span>{t('sidebar.categoryNav')}</span>
          </span>
          <button
            onClick={onToggleCollapse}
        ...
        <div className="flex items-center justify-between pt-0.5">
          ...
          >
            <option value="date_desc">{t('sidebar.sortNewest')}</option>
            <option value="date_asc">{t('sidebar.sortOldest')}</option>
          ...
          {hasActiveFilters && (
            ...
            >
              <X className="w-3 h-3" />
              <span>{t('sidebar.resetFilters')}</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-1">
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <span>⭐ {t('sidebar.starredFavorites')}</span>
          </div>
          <span className={`text-[10px] px-1.5 py-0.5 rounded ${
            filterRule.onlyFavorites
        ...
        <div>
          ...
          >
            <div className="flex items-center space-x-1.5">
              ...
              <span className="font-semibold text-slate-700">📷 {t('sidebar.imageTypeFilter')}</span>
            </div>
            <span className="text-[10px] text-slate-400 font-mono">{Object.keys(categoryStats.types).length}</span>
          </button>
          ...
          {openSections.types && (
            <div className="pl-4 pr-1 py-1 space-y-0.5 border-l border-slate-200 ml-3.5 mt-0.5">
              ...
              {Object.keys(categoryStats.types).length === 0 && (
                <div className="text-[11px] text-slate-400 py-1 pl-2">...</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="pt-1">
          ...
          >
            <div className="flex items-center space-x-1.5">
              ...
              <span className="font-semibold text-slate-700">🎨 {t('sidebar.artStyleFilter')}</span>
            </div>
            <span className="text-[10px] text-slate-400 font-mono">{Object.keys(categoryStats.styles).length}</span>
          </button>
        ...
        <div className="pt-1">
          ...
          >
            <div className="flex items-center space-x-1.5">
              ...
              <span className="font-semibold text-slate-700">🤖 {t('sidebar.modelFilter')}</span>
            </div>
            <span className="text-[10px] text-slate-400 font-mono">{Object.keys(categoryStats.models).length}</span>
          </button>
      ...
      <div className="p-3 border-t border-slate-200 text-[10px] text-slate-500 flex items-center justify-between bg-slate-50">
        <span>{t('app.pipelineFooter')}</span>
        <span className="text-emerald-600 font-medium font-mono">{t('app.offlineReady')}</span>
      </div>
    </aside>
  );
};
